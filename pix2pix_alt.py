...
def my_train(args):
    ...
    #generator_model.load_weights('params_generator_starGAN_epoch_391.hdf5')
    # load discriminator model
    discriminator_model = models.my_load_DCGAN_discriminator(img_shape, disc_img_shape, patch_num)
    #discriminator_model.load_weights('params_discriminator_starGAN_epoch_391.hdf5')
    ...
    for e in range(args.epoch):
        ...
        j += 1
        print("")
        print('j %d, Epoch %s/%s, Time: %s' % (j, e + 1, args.epoch, time.time() - starttime))
        if j % 10==0:
            """
            plot_generated_batch(X_proc_batch, X_raw_batch, generator_model, args.batch_size, "training" +str(e+1))
            idx = np.random.choice(procImage_val.shape[0], args.batch_size)
            X_gen_target, X_gen = procImage_val[idx], rawImage_val[idx]
            plot_generated_batch(X_gen_target, X_gen, generator_model, args.batch_size, "validation"+str(e+1))
            """
            generator_model.save_weights('params_generator_pix_epoch_{0:03d}.hdf5'.format(j), True)
            discriminator_model.save_weights('params_discriminator_pix_epoch_{0:03d}.hdf5'.format(j), True)
        else:
            continue
...
def main():
    ...
    parser.add_argument('--datasetpath0', '-d_train', type=str, default ="datasetbtrain.hdf5")  #,  "required=True)
    parser.add_argument('--datasetpath1', '-d_test', type=str, default ="datasetbtest.hdf5")   #, required=True)
